...
def read_all_xlsfiles_day(modelos,escenarios,path_in,var):


    list_file = {}
    esc = escenarios
    for mn in modelos[0:]:
        new_path = path_in/mn/var
        list_file[mn] = {esc[0]: pd.read_csv(os.path.join(path_in,  '01_Series_historical-obs_' +  var + '.csv'), index_col=0),
            esc[1]: pd.read_csv(os.path.join(new_path,  '02_Ds_Series_historical-gcm_' +  var + '.csv'), index_col=0).loc[:'31-12-2100'],
            esc[2]: pd.read_csv(os.path.join(new_path, '03_Ds_Series_ssp245_' +  var + '.csv'), index_col=0).loc[:'31-12-2100'],}
            # esc[3]: pd.read_csv(os.path.join(new_path, '04_Ds_Series_ssp370_' +  var + '.csv'), index_col=0).loc[:'31-12-2100']}
    return list_file
...
models = ['CanESM5','CMCC-ESM2','MIROC-ES2L','NorESM2-MM',]

escenarios = ['historical-obs','historical-gcm','ssp245', ]
...
for var in variables:
    ...
    if tipo == 'Monthly':
        ...
        path_in = path/'02_Series_Downscaling/'
        output_path = os.path.join(path, '03_Ensamble_Series', 'Daily', var)
        if not os.path.exists(output_path):
            os.makedirs(output_path)
